...
- **Against Laya with its head fine-tuned on the same examples:** MahaBodi beats it on emotion, banking77, ties on ag_news, boolq, **loses on sst5, prompt_injections (test items only)**.
...
| boolq | Ubuntu GPU, re-run (math SDP) | (0.0, 0) | 0.824 | 0.824 | 0 / 0, p 1 | 0.832 | 11 / 7, p 0.481 | tie (fine-tuning gave no validation gain, so fine-tuned = zero-shot) | 0/500 test, 0/500 fresh |
